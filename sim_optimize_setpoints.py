...
def evaluate_simulation(seq, log_path=None, zone_name=None,
    ...
    Returns: score, normalized_energy, normalized_comfort
    """
    ...
    T_out = df_result["T_out"].values if "T_out" in df_result.columns else np.full_like(T_in, 30.0)

    comfort_penalty = 0.0
    for t_in, t_out in zip(T_in, T_out):
        if t_out >= 20.0:
            comfort_penalty += (max(0, target_min - t_in) ** 2 + max(0, t_in - target_max) ** 2)
...
def find_best_setpoint_by_simulation(log_path=None,
    ...
    def fitness(x):
        ...
        # 3. 평가
        score, _, _ = evaluate_simulation(seq,
    ...
    varbound = np.array([[0, 2]] * 4)  # 각 위치는 {0,1,2} 정수 (22.0, 23.0, 24.0에 대응)
    ...
    # 최종 해 재평가 (score, energy, comfort 세부 출력 포함)
    x = model.output_dict["variable"]
    best_seq = [value_map[int(i)] for i in x]

    latest_log = find_latest_phi_gpt_log_path(base_log_path)
    best_score, energy_score, comfort_score = evaluate_simulation(
        best_seq,
        log_path=latest_log,
        zone_name=zone_name,
        w_energy=w_energy,
    ...
    return best_seq, best_score, energy_score, comfort_score


if __name__ == "__main__":
    best_seq, score, energy, comfort = find_best_setpoint_by_simulation()
    print("\n✅ Best GA Setpoints:", best_seq)
    print(f"⚖️  Normalized Score: {score:.4f}")
    print(f"   🔋 Energy Component: {energy:.4f}")
    print(f"   🌡️ Comfort Component: {comfort:.4f}")
